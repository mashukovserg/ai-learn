...
export default async function IdeasHistoryPathPage(props: {
  params: Promise<{ lang: string }>;
}) {
  const { lang } = await props.params;
  const l = lang as 'en' | 'ru';
  const rooms = getRoomsByPath('ideas-history');
  const pathMeta = PATHS_METADATA.find(p => p.id === 'ideas-history')!;

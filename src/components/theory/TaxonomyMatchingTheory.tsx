    ...
    <>
      ...
      <div className="bg-card-dark border border-border-card rounded-xl p-8 mb-8">
        ...
        <div className="space-y-6">
          <p className="text-neutral-300 leading-relaxed text-lg">
            {lang === 'ru'
              ? 'Семантический слой сравнивает не строки, а смысл. Каждое название вакансии и каждый профиль профессии превращаются в эмбеддинг — вектор чисел, где близкие по смыслу тексты оказываются рядом в пространстве. Ключевой трюк для нашей задачи — мультиязычная модель эмбеддингов (например, LaBSE или multilingual-e5), которая кладёт русский и английский в одно общее пространство. Тогда русская профессия и английская вакансия сравниваются напрямую, и машинный перевод не нужен вовсе. Близость измеряют косинусным сходством (cosine similarity): чем ближе векторы, тем выше оценка.'
              : 'The semantic layer compares meaning, not strings. Each vacancy title and each profession profile is turned into an embedding — a vector of numbers where texts close in meaning end up near each other in space. The key trick for our task is a multilingual embedding model (for example LaBSE or multilingual-e5) that places Russian and English in one shared space. A Russian profession and an English vacancy can then be compared directly, and machine translation is not needed at all. Closeness is measured by cosine similarity: the nearer the vectors, the higher the score.'}
          </p>
          <p className="text-neutral-300 leading-relaxed">
            {lang === 'ru'

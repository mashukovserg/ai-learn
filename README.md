...
- Rooms catalog (`/${lang}/rooms`) now uses a responsive tile grid layout (3 columns from `1200px`, 2 from `768px`, 1 on mobile) with equal-height cards, filter controls for difficulty/focus/progress status (focus supports thematic presets like `Agent Coding` and `AI Philosophy` plus all room categories), and a compact meta row for difficulty + status above each title.
- Room cards were rebuilt as the "quiet card" (Fork 5 resolved in `docs/DESIGN_FORKS.md`, 2026-08-23): the gradient cover, the `%` ring and the cyan hover are gone; a card is now a category-tinted icon tile + mono category label, title, 2-line description, a mono meta row (difficulty dot · time · `N/M` solved or `✓ completed`), and a 2px progress bar on the bottom edge that only appears once progress is above zero. Card height dropped from ~340px to ~190px.
...
- Rooms pages (49 rooms):
  - `/${lang}/rooms` (listing page with live progress states, thematic focus filter, and compact difficulty/status metadata)
  - `/${lang}/rooms/[id]` — Dynamic room renderer for 49 topics across foundations, debates, advanced systems, agent coding, and open models, including the Open Models pair (`local-models-101`, `llama-3-1-8b`) and the `ai-career-trajectories` room with a dedicated career-roadmap theory layout.

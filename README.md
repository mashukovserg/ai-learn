...
- Rooms pages (48 rooms):
  - `/${lang}/rooms` (listing page with live progress states, thematic focus filter, and compact difficulty/status metadata)
  - `/${lang}/rooms/[id]` — Dynamic room renderer for 48 topics across foundations, debates, advanced systems, agent coding, and open models, including the Open Models pair (`local-models-101`, `llama-3-1-8b`) and the `ai-career-trajectories` room with a dedicated career-roadmap theory layout.

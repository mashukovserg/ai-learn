...
- **Interpretability cluster — sourced from the AI Interpretability School curriculum table (ai-interpretability.school, reviewed 2026-08-20).** That course's deep-math track (SHAP derivations, CAM family, IG/DeepLIFT/LRP axioms, causal inference) is the wrong altitude for ai-learn rooms and is better linked than duplicated; what maps well is the conceptual layer:
  - **LLM Interpretability: From Black Box to Circuits** (`llm-interpretability`) — Path: pairs with `llm-mechanics` and `ai-alignment`. Topics: why "black box" is a trust and regulation problem (bridges to `ai-regulation-eu`/`ai-regulation-ru`); local vs global, intrinsic vs post-hoc explanations as a two-axis map; probing and the logit lens conceptually; superposition and why one neuron rarely means one concept; sparse autoencoders and feature steering told through public artifacts (Anthropic's Golden Gate Claude, Gemma Scope, Neuronpedia — all screenshot-friendly primary sources); the honesty caveat that an explanation itself can be wrong (sanity checks, ROAR/ROAD in one paragraph, not the math).
    Task types: categorize (explanation methods on the two-axis map), mentor (a colleague treating an attention map as proof of what the model "thinks"), timeline (probing → logit lens → SAE features), scenario (a regulator asks "why did the model refuse this loan?").
  - **Reading Agent Traces** (`agent-trace-reading`) — Path: agent-coding, right after `claude-code-agentic-loop`. The idea the course's module 16 validates: which tool did the agent call and why, what the reasoning trace does and does not prove, failure taxonomy from real traces (wrong tool, right tool wrong args, hallucinated tool result, goal drift). ai-learn already teaches the loop; this room teaches the forensics of it. Task types: sorting (a scrambled real trace), categorize (failure types), mentor (trace looks fluent but the tool result was fabricated).
  - Chapter-sized borrowings, not rooms: attacks on explanations → a chapter in `ai-security`; "is this explanation even true" → a chapter in `prompt-evals` when it gets its depth pass.
  - Format note: their "course as a galaxy" interactive map is the same direction as `docs/ROADMAP_VIEW_MODE.md`; their dated release-roadmap block ("opens 2026-08-24, tell me when") is a retention pattern ai-learn could reuse once there is anything scheduled.

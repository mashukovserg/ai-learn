...
- **Room pages implemented:** 43 in `ROOMS_METADATA`, all 43 fully wired (`ai-literature-review` landed 2026-07-31, two-way cross-linked with `deep-search-agents`)
...
| Date | Milestone |
|------|-----------|
| 2026-07-31 | New room `ai-literature-review` (Intermediate, Practice; `agentic-systems` path after `deep-search-agents`): running a rigorous literature review with AI tools. 5 plain-prose chapters (review types + the hallucinated-citation risk, PICO framing + inclusion/exclusion criteria before searching, PRISMA-style screening + deduplication, citation verification against ghost references, synthesis + evidence tables + audit trail) + 10 tasks across 7 types. Two-way cross-linked with `deep-search-agents`. Room count 42→43; gate green (1970). |

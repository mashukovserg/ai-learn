...
- **Localization:** Done (`ru`/`en` routes + `proxy.ts` redirect to `/ru`). The locale contract lives in `src/types/lang.ts` — narrow untrusted locale strings once with `toLang`, then pass `Lang`; no `as 'en' | 'ru'` casts.
- **Room engine:** Done (`TaskQuestion` supports input, single-choice, multi-select; `TaskSorting` supports drag-to-reorder; `TaskMentor` supports dialogue). Dispatch lives in `TaskRenderer`; the room page's task state machine lives in `useRoomTasks`.
...
- **Room pages implemented:** 50 in `ROOMS_METADATA`, all 50 fully wired (`opencode-terminal-agent` added 2026-08-22; `research-ai-era` wired up 2026-08-18; `ai-literature-review` absorbed the orphaned SLR-tooling chapters the same day)
...
- **Rooms list page:** Shows real progress status (Completed / In Progress / Not Started), supports thematic focus filtering (presets `Agent Coding`, `AI Philosophy`, plus all room categories), and renders the Fork 5 "quiet card" — category-tinted icon tile, mono meta row with the solved-task count, and a bottom-edge progress bar (no gradient cover, no `%` ring).
...
| Date | Milestone |
|------|-----------|
| 2026-08-26 | Skill-formation evidence added to three rooms, from a Hacker News piece the owner sent (Lars Faye, «AI Coding will Prevent Expertise», 2026-07-22) — cited from the primary sources it rests on, not from the essay. `ai-career-trajectories` gained Chapter 5 «Экспертиза набирается трением» / «Expertise is built out of friction» (PNAS 2025 Bastani et al.: +48% GPT Base / +127% GPT Tutor in practice, −17% on the exam once access was removed; ICER 2024 Prather et al.: 21 eye-tracked lab sessions, the «illusion of competence»; Anthropic 2026: 52 engineers, 50% vs 67%, d = 0.738, p = 0.01) plus tasks 11–12; `agent-coding-foundations` gained Chapter 8 «Что нельзя отдавать агенту» / «What Not to Hand to the Agent» (the premise under the whole validation loop, cognitive debt vs offloading, Spolsky's Law of Leaky Abstractions) plus tasks 14–15; `frontier-evals-logic` gained «Навык или способность его набирать» / «Skill, or the Ability to Acquire It» (Chollet, «On the Measure of Intelligence», 2019 — skill can be bought with priors and data, so measure skill-acquisition efficiency) plus task 14 and a cross-link to the career room. 3 new glossary terms. Gate green (2550). |
| 2026-08-23 | `ai-career-trajectories` rebuilt as a lesson: four sourced chapters (≥240 words / ≥4 paragraphs per language) around the existing interactive career map — published career frameworks (Dropbox IC1–IC7 vs MLE IC1–IC5, progression.fyi), what scope/reach/levers actually change between levels, Larson's four staff archetypes, the AI-engineer / ML-engineer / researcher split per swyx against Anthropic's combined RE/RS posting, market numbers from Lightcast for the 2026 AI Index, and Majors' engineer/manager pendulum with a two-week test protocol. Bilingual Sources card with 8 verified references; tasks 6 → 10; 6 new glossary terms; room time 20 → 35 min; new room-level guard. Gate green (2365). |
| 2026-08-23 | Fork 5 resolved — the `/rooms` catalog now ships variant B, the "quiet card" (`docs/room-card-picker.html`, owner's pick). Gradient cover, `%` ring, corner category chip and the cyan lift-on-hover removed; the card is a category-tinted icon tile + mono category label, title, 2-line description and a mono meta row (difficulty dot · time · `N/M` solved / `✓ completed`), with a 2px progress bar on the bottom edge that renders only above 0%. The whole card is the link now that the footer arrow is gone; `getCoverTone`/`progressTone` deleted in favour of `getCategoryTone`/`difficultyDot`; the progress snapshot carries solved/total, not just a percentage; `--color-violet-400` added to the light theme so the Ideas tile stays readable. Height ~340px → ~190px. Verified in the browser at RU/EN × dark/light × 0%/N%/100% and at 375px. Gate green (2339). |
| 2026-08-22 | New room `opencode-terminal-agent` (Intermediate, agent-coding path, right after `agentic-cli-tools`): OpenCode as the open-source (MIT), model-agnostic counterpart to the Claude Code rooms. 5 bilingual chapters (what OpenCode is; the build/plan permission split; `/init` → AGENTS.md, `/undo` checkpoints, `/timeline`, `/review`; a close reading of a real session; OpenCode Zen + stealth models with the data-clause tradeoff) + 10 tasks across 7 types. Three genuine captures of the owner's own OpenCode session editing this repo's backlog serve as the exhibits; facts verified same-day against opencode.ai docs and the GitHub repo. Room count 49→50; gate green (2394). |
| 2026-08-20 | Responsive pass across the app (audited 375/768/1280; iframe probe of all 49 rooms + 13 pages at 375px → 0 horizontal overflows): mobile navbar search panel, stacked dashboard hero, SkillsMatrix mobile layout (chip-row role picker, wrapping header), RadarChart label clipping fix, room-page floating "Задания N/M" jump pill + no nested task-panel scroll below `lg`, `.reading-prose` heading flex-wrap guard (fixed 3 overflowing rooms at once), TaskCategorize `break-words`, and RU/EN parity for hardcoded English labels in `TaskQuestion`/`TaskSorting`. Fork 5 (room card design) opened with `docs/room-card-picker.html` — 4 variants on real content; decision pending. Gate green (2339). |
| 2026-08-10 | New room `search-retrieval-to-synthesis` (Intermediate, ideas-history path, placed right after `chatgpt-moment`): what the ChatGPT break did to one concrete institution — web search. 6 bilingual chapters (list-as-division-of-labour; the generative pipeline; the Shah & Bender diagnosis written before ChatGPT; the measurements — 51.5%/74.5% verifiability, the CHI 2024 echo chamber, why users switch; the economics of sources; a verification protocol) + 11 tasks across 7 types; 5 new glossary terms; one real Wikimedia Statistics exhibit. Room count 47→48; gate green (2247). |
| 2026-08-10 | Frontend structural refactor (no content text touched): locale contract centralized in `src/types/lang.ts` — ~40 unchecked `as 'en' \| 'ru'` casts across 13 files replaced by a single `toLang` narrowing at each boundary; `rooms/[id]/page.tsx` decomposed 355 → 188 lines into `TaskRenderer`, `useRoomTasks` and `lib/taskDoneSound`. New `lang` test suite; gate green (2247). |
| 2026-08-07 | Two new rooms close the roadmap's overdue M2-1 head: `agentic-guardrails` (AC-301, Intermediate, agent-coding path, right after AC-205) — policy as a checkable list, the four red-team directions, findings turning into an adversarial eval set inside the quality gate, deny-by-default boundaries that never ask the model, and post-release monitoring with the precision/recall trade-off; and `agentic-cost-latency` (AC-302, Intermediate, right after AC-301) — cost per run rather than per request, TTFT vs total latency and P95 tails, four levers with the price of each, per-run/project/provider ceilings with a circuit breaker, and tracing that attributes spend. 5 bilingual chapters each (all ≥4 paragraphs, ≥240 words per locale) with one terminal and one real screenshot exhibit apiece (OWASP Top 10 for LLM Applications; Artificial Analysis time-to-first-token measurement); 12 tasks of 7 types each. 24 new glossary entries — 15 of them repairing `[Term not found]` markers that were live on 6 room pages. Room count 47→49; gate green (2288). |
| 2026-08-02 | New room `ai-alignment-limits` (Intermediate, ideas-history path): Schuster & Kilov 2025 — the dual challenge (epistemic justification + political legitimacy) against crowdsourcing/RLHF/constitutional AI on morally contested decisions. 5 bilingual chapters + 12 tasks; `value-alignment` glossary term. Room count 46→47; gate green (2178). |
| 2026-07-29 | Two new rooms closing the git/GitHub gap in the agent-coding path: `git-safety-net` (AC-105, Beginner, right after AC-102) — reading a diff you did not write, commits as units of undo, restore/stash/reset/revert/reflog by where the change lives, branch + worktree per task; and `github-actions-ci` (AC-206, Intermediate, right before AC-205) — clean-machine CI, workflow anatomy (triggers/jobs/steps), cache + parallel jobs + matrix + path filters, secrets and run permissions with the "agent does not edit workflows" rule, artifacts and reading a red run. 5 bilingual chapters each (all ≥4 paragraphs, ≥240 words per locale) with 4 + 3 terminal exhibits; 10 tasks of 6 types each; 12 new glossary terms (diff, commit, staging-area, revert, reflog, worktree, ci-cd, workflow, runner, matrix-build, build-artifact, ci-secrets). Room count 43→45; gate green (2068). |
| 2026-07-29 | New room `agentic-release-control` (AC-205, Intermediate, agent-coding path, after AC-202): the release layer the path was missing — local run vs shared CI, quality gates + branch protection and the three separate permissions (propose / start checks / merge), reviewing an agent's PR, canary + feature flags read against pre-set metric thresholds, rollback ladder and the autonomy budget. 5 bilingual chapters with 3 real screenshots (GitHub run awaiting maintainer approval, PR Files changed, Grafana service metrics) + 2 terminals; 11 tasks across 6 types; 1 new glossary term (`branch-protection`). Room count 42→43; gate green (1974). |
| 2026-07-23 | New room `local-rag-docs` (Intermediate, Open Models; beginner + intermediate paths): the first concrete-task local-models room — private RAG over one's own documents. 5 plain-prose chapters (index/query phases, multilingual embeddings vs the MiniLM trap + MTEB, same-model rule, Terminal build session with sourced answers, retrieval-first debugging, honest local-vs-cloud limits) + 11 tasks of 7 types. Room count 41→42; gate green (1923). |
| 2026-07-22 | New room `ai-existential-risk` (Intermediate, ideas-history path): Bostrom's four classes of existential risk + maxipok, the basic AI x-risk case decomposed into three premises (Grace/AI Impacts), counterarguments sorted by premise, and the "proves too much" test. 5 bilingual chapters + 12 tasks; both sources cited. Room count 40→41; gate green (1865). |
| 2026-07-21 | New room `context-engineering-101` (AC-204, Intermediate, agent-coding path): context layers, window economics (ROI test, context rot), knowledge artifacts (rule/fact/procedure/role → constitution/index/skill/agent), hygiene cycle with a terminal session, compaction. 5 bilingual chapters + 12 tasks across 7 types; 2 new glossary terms. Room count 39→40; gate green (1769). |
| 2026-07-16 | Roadmap M1-5 (pilot, awaiting creator approval): added the first task illustrations using the M1-4 infrastructure — design-token SVG concept diagrams on `agentic-cli-tools#1` (CLI loop), `claude-code-pro-workflow#5` (auto-compact buffer), and `claude-code-agentic-loop#8` (three-phase loop). Attached to context tasks (not the phase-naming MC or the loop-ordering sorting task, which a diagram would give away); bilingual alt/caption; gate green 1706. |
| 2026-07-19 | Roadmap M1-6 done — Month 1 fully closed. Added 5 Grok tasks across existing rooms, each solvable from that room's theory with Grok as the concrete example: `llm-landscape#16` (xAI tool-priced agentic API), `deep-search-agents#7` (realtime X-feed grounding, multiple-select), `native-multimodality#7` (MC with a bar-chart image via the M1-4 infra — native reading vs the old captioner chain), `prompting-101#11` (system prompt overrides Grok's default persona), `frontier-evals-logic#13` (interpret a vendor "Grok is #1" benchmark claim). Stable facts only, no fragile numbers. check-all green (1736). |
| 2026-07-16 | Roadmap N-2 + M1-4 closed: merged PR #1 to `main` (Vercel prod now carries all July work) and shipped the task-image infrastructure — optional `image` field on tasks (`src`, bilingual `alt`/`caption`), rendered via a shared `TaskIllustration` for all 8 task types, assets under `public/images/tasks/<room-id>/`, Vitest enforcement (missing file or missing locale fails `npm run test`), documented in `AGENTS.md`. Verified: image + localized alt/caption render in both locales; broken path fails the suite. |
| 2026-07-15 | Roadmap M1-1…M1-3 closed: fixed all 47 tracked data defects (22 categorize mappings normalized to English-keyed form, 6 scenario tasks rescaled to 0–100 with reachable passing choices, 19 missing bilingual explanations authored), shipped the `prompt-evals` task file (9 tasks, 7 types — room count now 39/39 wired), and armed the test gate: `npm run check-all` = lint + typecheck + 1700 tests, all green. |
| 2026-07-15 | New room `taxonomy-matching` (Intermediate, "Practice"): matching messy job titles to a curated profession/skill taxonomy as an applied ML task — normalization, lexical vs. semantic matching, multilingual embeddings, the cheap→precise cascade with LLM adjudication, and precision/recall evaluation. 5 bilingual theory chapters + 10 tasks across 7 types. Room count 38→39 (38 wired). |
| 2026-07-15 | Hardened `TaskCategorize` tap-to-place for multi-item buckets: tapping a bucket that already held a chip could hit the chip's remove handler instead of adding; now a tap on a placed chip while an item is selected places into that chip's bucket. Verified by solving a 3-items-per-bucket task on a touch viewport. |

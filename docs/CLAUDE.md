...
TryHackMe-style interactive AI education platform with 49 rooms, 3 learning paths, and full bilingual (RU/EN) support. Guest mode works out of the box; authenticated users get persistent progress synced to PostgreSQL.
...
**Next.js App Router** — App Router's folder-based routing maps cleanly to URLs and makes the `[lang]` dynamic segment the natural mechanism for bilingual routing. All 49 rooms share a single `rooms/[id]/page.tsx` renderer instead of 49 files — room data is looked up at runtime from `src/data/rooms/`.
...
│   ├── metadata.ts          # Room catalog (49 rooms: title, desc, difficulty, paths)

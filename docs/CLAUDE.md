...
TryHackMe-style interactive AI education platform with 52 rooms, 3 learning paths, and full bilingual (RU/EN) support. Guest mode works out of the box; authenticated users get persistent progress synced to PostgreSQL.
...
**Next.js App Router** — App Router's folder-based routing maps cleanly to URLs and makes the `[lang]` dynamic segment the natural mechanism for bilingual routing. All 52 rooms share a single `rooms/[id]/page.tsx` renderer instead of 52 files — room data is looked up at runtime from `src/data/rooms/`.
...
│   ├── metadata.ts          # Room catalog (52 rooms: title, desc, difficulty, paths)
